...
import com.shizukio.mc.warpmarker.commands.WarpMarkerCommands
import com.shizukio.mc.warpmarker.debug.DebugRenderer
import com.shizukio.mc.warpmarker.services.MarkerService
import com.shizukio.mc.warpmarker.state.PlayerState
import com.shizukio.mc.warpmarker.utils.AdventureMessageSender
import com.shizukio.mc.warpmarker.utils.AdventureMessages
import com.shizukio.mc.warpmarker.utils.Console
import com.shizukio.mc.warpmarker.utils.LegacyAdventureMessageSender
 ...
 *
 * この class は warpmarker-bukkit module にあります。
 * つまり Bukkit API / CommandAPI に依存してよい層です。
 *
 * warpmarker-core は loader 非依存の model / enum だけを持ちます。
 * JavaPlugin、CommandAPI、Bukkit scheduler などは core に置きません。
 *
 * Paper は Bukkit 互換なので、この Bukkit 実装を土台にできます。
 * Paper 固有 API が必要になったら warpmarker-paper 側へ adapter を追加します。
 */
object WarpMarkerBukkitRuntime {
    ...
    fun onLoad(
        ...
        // /warpmarker list のクリック・hover などは、この sender が Component を保持できるかに依存します。
